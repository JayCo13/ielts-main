...
const Reading_Fe = () => {
  ...
  // Deep-link from the public SEO landing pages: /reading_list?open=<exam_id>
  // auto-launches that full test once the access-filtered list has loaded.
  // If the id isn't in `tests` (no access / VIP-only) we do nothing and the
  // normal list + VIP gating shows — the blur/lock logic is untouched.
  ...
  useEffect(() => {
    ...
    if (target) {
      ...
      navigate('/reading_test_room', { state: { examId: target.id } });
    }
